...
public class DashboardController implements Initializable {
	...
	private boolean forexFlag = true;

	private boolean threadQuotes = false;

	private Article articleTrendingContainer = null;
	private Article articleContainer = null;
	private Newsource newssourceContainer = null;
	private Stock stockHolder;

	private static Set<Stock> generalStocks = new HashSet<>();
	private boolean threadGeneralStocks = false;

	@FXML
	private JFXToggleButton articlePerspectiveButton;
	@FXML
	private JFXToggleButton stockPerspectiveButton;
	...
	@FXML
	ToggleButton generateInfoButton;

	/* STOCK INFO */
	@FXML
	AnchorPane stockInfo;
	@FXML
	LineChart stockLineChart;
	@FXML
	Label coporationNameLabel;
	@FXML
	Label coporationSymbolLabel;
	@FXML
	Label coporationHighLabel;
	@FXML
	Label coporationLowLabel;
	/* END STOCK INFO */
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		String jndiName1 = "infinity_server-ear/infinity_server-ejb/ServiceNewsArticle!tn.esprit.infinity_server.interfaces.NewsArticleRemote";
		String jndiName2 = "infinity_server-ear/infinity_server-ejb/ServiceStock!tn.esprit.infinity_server.interfaces.StockRemote";
		try {
			...
			LOGGER.log(Level.SEVERE, "done building the stock proxy");
		} catch (NamingException e) {
			LOGGER.log(Level.SEVERE, "Could not build the proxies");
		}
		prepareArticles();
		prepareNewsSources();
		prepareTableViews();
	...
	void prepareTableViews() {
		...
		stockTable.getSelectionModel().selectedItemProperty().addListener((ObservableValue<? extends Stock> observable,
				Stock oldValue, Stock newValue) -> stockHolder = newValue);

		mystockTable.getSelectionModel().selectedItemProperty()
				.addListener((ObservableValue<? extends Stock> observable, Stock oldValue,
						Stock newValue) -> stockHolder = newValue);
	...
	void prepareTerminal() {
		...
		Timeline quotesCallBackerSymbols = new Timeline(new KeyFrame(Duration.seconds(100), (ActionEvent event) -> {
			if (terminalFlag) {
				if (stockFlag && !forexFlag) {
					buildStockQuotes();
				} else {
					buildQuotes();
				}
	...
	void preparePerspective() {
		...
		generateInfoButton.setVisible(false);
		...
		stockPerspectiveButton.setOnAction(e -> {
			...
			if (!(stockPerspectiveButton.isSelected())) {
				...
				refreshArticlesButton.setVisible(false);
				generateCommentButton.setVisible(false);
				generateInfoButton.setVisible(true);
	...
	void prepareArticles() {
		...
		stockProxy.getAllStock().forEach(s -> myStocksObservableList.add(s));
		savedArticles = articleProxy.getAllArticles();
		LOGGER.log(Level.SEVERE, "building trending articles...");
	...
	void buildNewsSources(boolean updateFlag) {
		...
		newsSources.getSelectionModel().selectedItemProperty()
				.addListener((ObservableValue<? extends Newsource> observable, Newsource oldValue,
						Newsource newValue) -> newssourceContainer = newValue);

		newsSources.setCellFactory(v -> new SimpleMediaListCell<>());
		newsSources.setItems(dataNewsSource);
		LOGGER.log(Level.INFO, "****** done building news sources *****");
	}
	...
	void buildTrendingArticles() {
		...
		try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				a1.setPublishedAt(jsonobject.getString("publishedAt"));
			...
			articlesTrending.getSelectionModel().selectedItemProperty().addListener(
					(ObservableValue<? extends Article> observable, Article oldValue, Article newValue) -> {
						...
						try {
							...
							LOGGER.log(Level.SEVERE, "could not fetch articles from selected news source.");
						}
					});
		...
		LOGGER.log(Level.INFO, "***** done fetching trending articles from the web *****");
	}
	...
	void buildArticles(boolean flagMy, String url) {
		...
		if (!flagMy) {
			if (cachedArticles.isEmpty()) {
				...
				LOGGER.log(Level.INFO, "****** done building articles from web ******");
			} else {
				cachedArticles.forEach(a1 -> {
					...
					try {
						...
						LOGGER.log(Level.SEVERE, "could not fetch image.");
					}
					String titleToLabel = (a.getTitle().length() > 50) ? a.getTitle().substring(0, 50) + "..."
							: a.getTitle();
					String authorToLabel = (a.getAuthor().length() > 50) ? a.getAuthor().substring(0, 50) + "..."
					...
					link.setOnMouseClicked((MouseEvent event) -> {
						try {
							...
							LOGGER.log(Level.SEVERE, "could not load article url.");
						}
					});
				...
				LOGGER.log(Level.INFO, "****** done building articles from cache ******");
			}
		} else {
			...
			LOGGER.log(Level.INFO, "****** done building my articles ******");
		}
	}
		...
		if (!threadGeneralStocks) {
			...
			for (int i = 0; i < symbolsContainer.length; i++) {
				...
				if (jsonObject.has("Monthly Time Series")) {
					...
					LOGGER.log(Level.SEVERE, "Symbol not fetched by Alpha Vantage");
			}
			generalStocksObservableList = FXCollections.observableArrayList();
			generalStocks.forEach(s -> generalStocksObservableList.add(s));
	...
	void generateStockComment() {
		...
		if (result.isPresent() && result.get() == ButtonType.OK) {
			...
			myStocksObservableList.clear();
			stockProxy.getAllStock().forEach(s -> myStocksObservableList.add(s));
			Alert alertConfirmationSpeech = new Alert(Alert.AlertType.CONFIRMATION);
			alertConfirmationSpeech.setTitle("Confirmation");
			alertConfirmationSpeech.setHeaderText("Addition");
			alertConfirmationSpeech.setContentText(
					"Do you want the AI to speak?\nYou should listen to whole speech once you agree.");
			titleMain.setText("Please listen to the speech");
			Optional<ButtonType> resultSpeech = alertConfirmationSpeech.showAndWait();
			if (resultSpeech.isPresent() && resultSpeech.get() == ButtonType.OK) {
				goSpeak(comment);
			}
			mystockTable.setVisible(true);
			buildMyStocks();
	...
	void goSpeak(String comment)
	{
		 System.setProperty("freetts.voices",
		 "com.sun.speech.freetts.en.us.cmu_us_kal.KevinVoiceDirectory"); try {
		 Central.registerEngineCentral(
		 "com.sun.speech.freetts.jsapi.FreeTTSEngineCentral"); Synthesizer synthesizer
		 = Central.createSynthesizer(new SynthesizerModeDesc(Locale.US));
		 synthesizer.allocate(); synthesizer.resume();
		 synthesizer.speakPlainText(comment, null);
		 synthesizer.waitEngineState(Synthesizer.QUEUE_EMPTY);
		 synthesizer.deallocate(); } catch (EngineException | AudioException |
		 EngineStateError | IllegalArgumentException | InterruptedException e1) {
		 LOGGER.log(Level.SEVERE, "error: Robot could not speak"); }
	}
	@FXML
	void generateStockInfo() {
		if (!generateInfoButton.isSelected()) {
			titleMain.setText("Stocks");
			stockInfo.setVisible(false);
			buildStocks();
			return;
		...
		mystockTable.setVisible(false);
		stockTable.setVisible(false);
		titleMain.setText("Stock Data");
		stockInfo.setVisible(true);
		stockLineChart.setTitle("Stock Monitoring, 2017");
		coporationNameLabel.setText("Corporation: " + stockHolder.getName());
		coporationSymbolLabel.setText("Corporation's Symbol: " + stockHolder.getSymbol());

		XYChart.Series series = new XYChart.Series();
		series.setName(stockHolder.getName().toUpperCase());
		String response = HttpRequest.get("https://www.alphavantage.co/query?function=TIME_SERIES_MONTHLY&symbol="
				+ stockHolder.getSymbol() + "&interval=1min&apikey=R5Y2Z66NSW3CFTXA").accept("application/json").body();
		JSONObject jsonObject = new JSONObject(response);

		TreeSet<String> sortedKeysToFetchFrom = new TreeSet<>(new MyCompReversed());
		Set<String> keysToFetchFrom = ((JSONObject) jsonObject.get("Monthly Time Series")).keySet();
		sortedKeysToFetchFrom.addAll(keysToFetchFrom);
		...
		for (String K : sortedKeysToFetchFrom) {
			...
			if (index > 12)
				break;
			LOGGER.log(Level.INFO, "step " + index + " chart....");
			JSONObject result = ((JSONObject) ((JSONObject) jsonObject.get("Monthly Time Series")).get(K));
			maxHigh = Float.max(maxHigh, ((float) (result.getDouble("2. high"))));
			minLow = Float.min(minLow, (float) (result.getDouble("3. low")));
			series.getData().add(new XYChart.Data(String.valueOf(index), (float) (result.getDouble("1. open"))));
			index++;
		}
		LOGGER.log(Level.INFO, "Process has ended.");
		stockLineChart.getData().add(series);
		coporationHighLabel.setText("Highest Value (Year): " + maxHigh);
		coporationLowLabel.setText("Lowest Value (Year): " + minLow);
	}
