...
class Terminal {
	...
	static final String MONTHYL = "Monthly Time Series";
	static final String UNKOWN = "unkown command... try the HELP command for guidelines";
	...
	private static String showStock(String symbol) {
		...
		Set<String> keysToFetchFrom = ((JSONObject) jsonObject.get(MONTHYL)).keySet();
		...
		JSONObject result = ((JSONObject) ((JSONObject) jsonObject.get(MONTHYL)).get(keys[0]));
